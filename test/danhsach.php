...
<div id="main" class="c-section text-center danhsach_acc">
	...
	if(count($ready_accs) > 0){
		...
		echo '<h2 class="c-items__title u-text-gradient">Tài khoản đang rỗi</h2>';
		$i = 0;
		foreach($ready_accs AS $acc){
			...
			echo '<div class="col-sm-3" id="acc_'.($this->gm_model->int_to_string($acc->acc_id)).'"><div class="acc_border"><div class="acc_inline">';
$hour = date("H", $acc->acc_time_ready);
if($hour == 22 OR $hour == 23 OR ($hour >= 0 AND $hour < 2)){
echo '<div class="pull-right trangthai" style="right:1px; left: auto; font-size: 12px">Chơi đêm+2h</div>';
}
			...
			if($acc->acc_time_ready == 0 || $acc->acc_time_ready <= time()){
				echo '<div class="ready_acc">Chiến được luôn</div>';
			} else {
				echo 'Chờ đổi pass: <span class="countdown_time">'.date("m/d/Y H:i:s", $acc->acc_time_ready).'</span>';
			}
			echo '</div>';
			...
			<?php
			if($acc->acc_time_ready == 0 || $acc->acc_time_ready <= time()){
				echo '<button type="submit" class="my_button">Thuê</button>';
			} else {
				echo '<button type="submit" class="my_button">Đặt trc</button>';
			}
			?>
	...
	if(count($accs) > 0) {
		usort($accs, 'cmp');
		echo '<h2 class="c-items__title u-text-gradient">Tài khoản sắp hết giờ </h2>';
		echo '<div>Sắp xếp theo thứ tự thời gian từ gần tới xa</div>';
		$i = 0;
		$previos_stop_time = 0;
		foreach($accs AS $acc){
			...
			if($trangthai != ''){
				echo '<div class="col-sm-3" id="acc_'.($this->gm_model->int_to_string($acc[0]->acc_id)).'"><div class="acc_border"><div class="acc_inline" style="background: #6f0909">';
			} else {
...
echo '<div class="pull-right trangthai" style="right:1px; left: auto; font-size: 12px">Chơi đêm+2h</div>';
			...
			echo '<strong>Tài khoản '.($this->gm_model->int_to_string($acc[0]->acc_id)).'</strong>';
				
				echo '<div class="acc_stt">';
				if($trangthai == ""){
					echo 'Còn lại: <span class="countdown_time">'.date("m/d/Y H:i:s", $acc[count($acc)-1]->h_stop_time).'</span> <br/>+'.(AFTER_TIME).'p chờ đổi pass<br/>';
					echo 'Thuê đc lúc '.date("H:i d/m", $acc[count($acc)-1]->h_stop_time+AFTER_TIME*60).'<br/>';			
				} else {
				echo '<a href="'.site_url('home/dathue').'">Click vào đây để xem chi tiết thời gian thuê và hết hạn.</a>';
				}
				...
				<?php } ?>
					...
					<button type="submit" class="my_button"><?php if(in_array($acc[0]->acc_id, $ds_thue) && $acc[0]->h_stop_time == $stop_time && $acc[0]->h_stop_time > time() - (BEFORE_TIME+5)*60 ){echo 'Gia hạn';} else {echo 'Đặt trc';}?></button>
			
				
				<?php 
				echo form_close();
				} else {
					echo '<button class="my_button">Đã có người đặt trc</button>';
				}
			...
			$.ajax({
				...
				success: function (e) {
				   swal({title: e.title, text: e.mess, type: e.stt, html:true},function(){ 
					   window.location.href = '<?php echo site_url('home/#stream');?>';
				   }
				   );
				}
			});
